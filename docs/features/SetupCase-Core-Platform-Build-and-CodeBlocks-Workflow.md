...
3. **`8-Save-CodeBlocks.php` references files that don't exist** in `sourceFiles/` (e.g. `AuditLogsTable.php`, `AuditContext.php`, `MenuStateHelper.php`, `Staff/AuditLogsController.php`, non-prefixed/Manager `EmailQueuesController.php`). The list is out of date with reality. **Fixed 2026-09-24:** the 8 dead entries were removed during the first `Update CodeBlocks` run. Still open: 47 synced CodeBlocks files (including RBAC) are not covered by any entry - see `docs/features/todo-fix-8-save-to-include-all-files.md`.
